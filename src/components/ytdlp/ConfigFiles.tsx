...
function buildDefault(port: number, cookie: string | null): string {
  return [
    ...
    '',
    '# === Video extraction ===',
    '# Try in order: [≤1440p, not upscaled] + [original audio, else best] → [≤1440p any] + [original audio, else best] → best original → best pre-merged',
    '-f "bestvideo[height<=1440][format_note!~=upscaled]+(bestaudio[format_note*=original]/bestaudio)/bestvideo[height<=1440]+(bestaudio[format_note*=original]/bestaudio)/best[format_note*=original]/best"',
